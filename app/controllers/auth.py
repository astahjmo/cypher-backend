...
from fastapi import Depends, HTTPException, status, Cookie, Request, Response, WebSocket # Added WebSocket for type hint if needed later
...
# This store needs a more robust implementation
...
async def _verify_token_and_get_user(
    ...
    credentials_exception = HTTPException(
        ...
        headers={"WWW-Authenticate": "Bearer"}, # Keep for HTTP context
    )
    if not token:
    ...
    try:
        if not settings.SECRET_KEY:
             logger.critical("JWT SECRET_KEY is not configured!")
             raise ValueError("JWT Secret Key is missing.") # Internal config error
    ...
    except ValueError as e: # Catch missing secret key
         logger.error(f"JWT configuration error: {e}")
         # Use 500 for internal config issues
         raise HTTPException(status_code=500, detail="Internal server error: JWT configuration.") from e
    except Exception as e:
        logger.error(f"Unexpected error decoding JWT: {e}", exc_info=True)
        raise credentials_exception from e # Treat unexpected decode errors as auth failure
    ...
    try:
        ...
        # Raise a generic 500 for DB errors during auth check
        raise HTTPException(status_code=500, detail="Error retrieving user data.") from e


# --- Secure Dependency to Get Current User (for HTTP Requests) ---
async def get_current_user_from_token(
    access_token: Optional[str] = Cookie(None, alias="access_token"), # Explicitly alias cookie name
    ...
    """
    logger.debug(f"Attempting to get user from cookie: {access_token[:10] if access_token else 'None'}")
    # Delegate verification to the shared function
    return await _verify_token_and_get_user(token=access_token, user_repo=user_repo)


# --- GitHub OAuth Logic Functions ---
...
def process_github_callback(
    ...
    """
    logger.info(f"Processing callback: code={code[:10]}..., state={state_from_gh}, state_key_cookie={state_key_from_cookie}")
    ...
    if not original_state or original_state != state_from_gh:
        logger.warning(f"State mismatch: Original='{original_state}', Received='{state_from_gh}'")
        raise ValueError("OAuth state mismatch.")

    try:
        github = OAuth2Session(settings.GITHUB_CLIENT_ID, state=original_state, redirect_uri=settings.GITHUB_CALLBACK_URL)
        token = github.fetch_token(token_url, client_secret=settings.GITHUB_CLIENT_SECRET, code=code)
        github_access_token = token.get('access_token')
        if not github_access_token:
             raise OAuth2Error(description="Failed to obtain GitHub access token.")
        logger.info(f"GitHub access token obtained successfully (first 5 chars): {github_access_token[:5]}...")

        user_response = github.get(github_api_url)
        user_response.raise_for_status()
        github_user_data = user_response.json()
        logger.info(f"Fetched GitHub user data for login: {github_user_data.get('login')}")
    ...
    except (OAuth2Error, requests.exceptions.RequestException, ValidationError, RuntimeError, ValueError) as e:
...
# --- Controller function stubs (called by views) ---

async def ctrl_handle_login_redirect() -> Tuple[str, str]:
    """Generates GitHub auth URL and state key."""
    return generate_github_auth_url()

async def ctrl_handle_github_callback(request: Request, user_repo: UserRepository) -> Tuple[User, str]:
    ...
    user, jwt_token = process_github_callback(code, state_from_gh, state_key_from_cookie, user_repo)
    return user, jwt_token
...
# --- Potentially needed for WebSocket Auth (if Depends doesn't work directly) ---
# This function is designed to be called manually within the WebSocket endpoint
async def get_user_from_websocket_cookie(
    websocket: WebSocket,
    user_repo: UserRepository # Manually pass the repository
) -> Optional[User]:
    """
    ...
    if not access_token:
        ...
        return None
    try:
        # Use the shared verification logic, catching potential HTTPExceptions
        user = await _verify_token_and_get_user(token=access_token, user_repo=user_repo)
        return user
    except HTTPException as e:
        # Log the specific auth error but return None to the WebSocket endpoint
        logger.warning(f"WebSocket cookie validation failed: {e.detail} (Status: {e.status_code})")
        return None
    except Exception as e:
        # Catch any other unexpected errors during verification
        logger.error(f"Unexpected error verifying WebSocket cookie token: {e}", exc_info=True)
        return None
